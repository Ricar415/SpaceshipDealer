...
// -- Constructor --
fighter::fighter(int dmaxspeed, int propulsiont, int pricet, weapon dweapon1, weapon dweapon2, owner *nowner, string n) : vehicle (n, nowner, propulsiont, 1, pricet, 3){
	maxspeed = dmaxspeed;
	weapon1 = dweapon1;
	weapon2 = dweapon2;
}
...
// -- Show functions --
void fighter::show() {
	cout << "Vehicle with register number: " << vrn << "  and Owner: " << vehicle_owner->rrn() << " Fighter;	Propulsion type: " << showpropulsion() << ", Maximum crew: 1, Price (billions): " << price << ", Maximum Speed : " << maxspeed << endl << "Weapon number 1: ";
	weapon1.show();
...
string fighter::reg() {
    ...
    return vrn + space + vehicle_owner->rrn() + space + t + space + p + space + mc + space + pr + space + ms + space + wt1 + space + wt2 + space;
}
