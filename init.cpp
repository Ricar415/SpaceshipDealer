	...
	if (!vehicle) { cerr << "Error: No vehicle register files encountered" << endl; }
		...
		while (!vehicle.eof()) { // duplication of the last line due to this practise will be ignored because no two vehicles can have the same rn
			...
			case 1 :
                ...
                if (!vehicle.fail()) { platforme->createcarrier(maxload, cs, es, propulsion, maxcrew, price, vrn); ii++;}
				break;
			case 2:
				vehicle >> size;
			...
			case 3:
				...
				w1.modify(wt1);
				w2.modify(wt2);
                if (!vehicle.fail()) { platforme->createfighter(maxspeed, propulsion, price, w1, w2, vrn); ii++; }
				break;
			case 4:
				vehicle >> maxp >> hn >> es;
                if (!vehicle.fail()) { platforme->createstation(maxp, hn, es, propulsion, maxcrew, price, vrn); ii++; }
				break;
			case 5:
				break;
			}
    ...
    if (!vehicle) { cerr << "Error: No vehicle register files encountered" << endl; }
        ...
        while (!vehicle.eof()) { // duplication of the last line due to this practise will be ignored because no two vehicles can have the same rn
            ...
            case 2:
                ...
                if (!vehicle.fail()) { platforme->createdestroyer(weapons, propulsion, maxcrew, price, vrn); ii++; }
                break;
            case 3:
                vehicle >> maxspeed >> wt1 >> wt2;
                w1.modify(wt1);
